...
def connect_to_snowflake(connection_parameters):
    ...
    """
    snowflake_session = sf.Snowflake(connection_params_dict=connection_parameters, client_name='RivalIQ')
...
def get_bulkSocialPosts(landscapeId, apiKey, companyId, 
                        mainPeriodStart='2023-01-01', 
                        mainPeriodEnd=datetime.today().strftime('%Y-%m-%d'),
...
def check_bulkDownload_status(downloadToken, apiKey, start_time):
    ...
    if response.status_code == 200:
        ...
        if status == 2:
            ...
            print(f"Download is still in progress. Checking again in 5 seconds... Elapsed time: {round(time.time() - start_time, 2)} seconds")
